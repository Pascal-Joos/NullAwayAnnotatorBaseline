...
def filter_errors(benchmark, version):
    ...
    if version == ADVANCED_VERSION:
        for metric_id in metrics_dict:
            if metric_id not in commits_dict:
                info = {
                    "id": metric_id,
                    "hash": "",
                    "type": "",
                ...
                errors[version][benchmark][metric_id] = info
    else:    
...
def main():
    ...
    print(f"Total errors solved by basic and agent but not advanced: {len(sample)}")
    ...
    for item in sample:
        keys = ["hash_advanced", "hash_basic", "hash_agent_baseline"]
        ...
        with open("manual_inspection_sample_unobfuscated.tsv", "a", encoding="utf-8") as out_file:
            writer = csv.writer(out_file, delimiter='\t')
            writer.writerow([item['benchmark'], item['id'], keys[0], keys[1], keys[2], make_github_url(item[keys[0]], item['benchmark'], 'Github'), make_github_url(item[keys[1]], item['benchmark'], 'Github'), make_github_url(item[keys[2]], item['benchmark'], 'Github'), item['type'], item['message'], item['path'], item['expression'], "", "", "", ""])
