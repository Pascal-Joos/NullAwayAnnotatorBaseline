...
def filter_errors(benchmark, version):
    ...
    for commit_id in commits_dict:
        ...
        if metric_row["TARGET_ERROR_RESOLVED_WITHOUT_NEW_ERRORS"].lower() == "true":
            info = {
                "id": commit_row["ID"],
...
def main():
    ...
    with open("manual_inspection_sample_unobfuscated_before.tsv", "r", encoding="utf-8") as before_file:
        csv.reader = csv.reader(before_file, delimiter='\t')
        # list all rows in the file that are not part of the sample
        before_rows = list(csv.reader)
        before_data_rows = before_rows[1:]
        before_sample_keys = set()
        for row in before_data_rows:
            before_sample_keys.add((row[0], row[1]))  # (benchmark, id)
        filtered_before_rows = []
        for row in before_data_rows:
            key = (row[0], row[1])
            if key not in [(item['benchmark'], item['id']) for item in sample]:
                filtered_before_rows.append(row)
    print(f"Rows in before file not in sample: {len(filtered_before_rows)}")
    # write thes rows to a new file
    with open("manual_inspection_sample_unobfuscated_excluded.tsv", "w", encoding="utf-8") as excluded_file:
        writer = csv.writer(excluded_file, delimiter='\t')
        ...
        for row in filtered_before_rows:
            writer.writerow(row)
